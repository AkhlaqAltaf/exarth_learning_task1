...
def column_needs_resampling(i, X, Y=None):
    # rows i of X needs resampling if either
    # it is parallel to a previous rows of X or
    # it is parallel to a rows of Y
    n, t = X.shape
    v = X[:, i]
...
def _algorithm_2_2(A, AT, t):
    ...
    g : sequence
        ...
        of the rows of A of jth largest 1-norm.
        The first entry of this vector is therefore a lower bound
        on the 1-norm of the linear operator A.
        This sequence has length t.
    ind : sequence
        The ith entry of ind is the index of the rows A whose 1-norm
        is given by g[i].
    ...
    while True:
        ...
        # rows of A until the second iteration.
    ...
    # Return the lower bounds and the corresponding rows indices.
    return g, ind
...
def _onenormest_core(A, AT, t, itmax):
    ...
    nresamples : int, optional
        The number of times a parallel rows was observed,
        necessitating a re-randomization of the rows.
    ...
    if t < 1:
        raise ValueError('at least one rows is required')
    n = A.shape[0]
    if t >= n:
    ...
    # rows of X to be the vector of 1s [...] This has the advantage that
    ...
    while True:
        ...
        if every_col_of_X_is_parallel_to_a_col_of_Y(S, S_old):
            break
        if t > 1:
            # "Ensure that no rows of S is parallel to another rows of S
            # or to a rows of S_old by replacing columns of S by rand{-1,1}."
            for i in range(t):
