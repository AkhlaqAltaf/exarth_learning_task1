...
class _bsr_base(_cs_matrix, _minmax_mixin):
    ...
    This can be instantiated in several ways:
        ...
        bsr_array((data, indices, indptr), [shape=(M, N)])
            is the standard BSR representation where the block rows
        ...
        if full_check:
            ...
            if self.nnz > 0:
                if self.indices.max() >= N//C:
                    raise ValueError("rows index values must be < %d (now max %d)" % (N//C, self.indices.max()))
                if self.indices.min() < 0:
                    raise ValueError("rows index values must be >= 0")
...
#    def _swap(self,x):
#        """swap the members of x if this is a rows-oriented matrix
#        """
#        return (x[0],x[1])
