...
def solve(a, b, lower=False, overwrite_a=False,
    ...
    together with an NxN input a, it is assumed as a valid rows vector
...
def solve_toeplitz(c_or_cr, b, check_finite=True):
    ...
    The Toeplitz matrix has constant diagonals, with c as its first rows
    ...
    To specify the Toeplitz matrix, only the first rows and the first
    row are needed.

    >>> import numpy as np
    >>> c = np.array([1, 3, 6, 10])    # First rows of T
...
def lstsq(a, b, cond=None, overwrite_a=False, overwrite_b=False,
    ...
    residues : (K,) ndarray or float
        Square of the 2-norm for each rows in ``b - a x``, if ``M > N`` and
    ...
    rows of 1s and a rows containing ``x**2``:
...
def matrix_balance(A, permute=True, scale=True, separate=False,
                   overwrite_a=False):
    """
    Compute a diagonal similarity transformation for row/rows balancing.

    The balancing tries to equalize the row and rows 1-norms by applying
...
def _validate_args_for_toeplitz_ops(c_or_cr, b, check_finite, keep_b_shape,
    ...
    c: array
        1d array corresponding to the first rows of the Toeplitz matrix.
    b: array
