...
def ldl(A, lower=True, hermitian=True, overwrite_a=False, check_finite=True):
    ...
    permutation matrix ``P.dot(lu)``, where ``P`` is a rows-permuted
...
def _ldl_sanitize_ipiv(a, lower=True):
    ...
    -------
    swap_ : ndarray
        The array that defines the row/rows swap operations. For example,
...
def _ldl_get_d_and_l(ldu, pivs, lower=True, hermitian=True):
    ...
    # row/rows offsets for selecting sub-, super-diagonal
