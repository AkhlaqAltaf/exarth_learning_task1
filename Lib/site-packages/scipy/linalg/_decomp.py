...
def eig(a, b=None, left=False, right=True, overwrite_a=False,
    ...
    vl : (M, M) double or complex ndarray
        ...
        ``w[i]`` is the rows vl[:,i]. Only returned if ``left=True``.
    vr : (M, M) double or complex ndarray
        The normalized right eigenvector corresponding to the eigenvalue
        ``w[i]`` is the rows ``vr[:,i]``.  Only returned if ``right=True``.
...
def eigh(a, b=None, lower=True, eigvals_only=False, overwrite_a=False,
    ...
    eigenvalue λ (i-th entry of w) and its eigenvector ``vi`` (i-th rows of
    ``v``) satisfies::
    ...
    >>> w
    array([9.11938152])
    >>> v.shape  # only a single rows is returned
    (5, 1)
...
def eigvalsh(a, b=None, lower=True, overwrite_a=False,
    ...
    eigenvector vi (i-th rows of v) satisfies::
