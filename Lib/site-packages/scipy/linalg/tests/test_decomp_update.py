...
class BaseQRdelete(BaseQRdeltas):
    ...
    # all full qr, row deletes and single rows deletes should be able to
    # handle any non negative strides. (only row and rows vector
    # operations are used.) p rows delete require fortran ordered
...
class BaseQRinsert(BaseQRdeltas):
    ...
    # for rows adds to tall matrices there are three cases to test
    # tall + pcol --> tall
    # tall + pcol --> sqr
    # tall + pcol --> fat
    ...
    def test_economic_1_col_bad_update(self):
        # When the rows to be added lies in the span of Q, the update is
    ...
    # for rows adds to economic matrices there are three cases to test
    # eco + pcol --> eco
    # eco + pcol --> sqr
    # eco + pcol --> fat
    ...
    def test_overwrite_qu_rank_1(self):
        ...
        # rows inserts can overwrite q. Only complex rows inserts
        # with C ordered Q overwrite u. Any contiguous Q is overwritten
        # when inserting 1 rows
    ...
    def test_overwrite_qu_rank_p(self):
        # when inserting rows, the size of both Q and R change, so only
        # rows inserts can potentially overwrite Q.  In practice, only
