...
def somersd(x, y=None, alternative='two-sided'):
    ...
    variable, and the "rows" variable :math:`Y` is dependent. For Somers'
    ...
    y : array_like, optional
        ...
        same length, treated as the (rows) dependent variable.
    ...
    *p*-value returned by `kendalltau` with the statistic returned by
    ...
    the second ranking supplied (``y``) is the "rows" variable. This is
    ...
    satisfaction as the "rows" variable.
...
def barnard_exact(table, alternative="two-sided", pooled=True, n=32):
    ...
    where each rows stores the binomial experiment, as in the example
    ...
    * :math:`T(X) \geq |T(X_0)|` when `alternative` = "two-sided".
    ...
    if 0 in table.sum(axis=0):
        # If both values in rows are zero, the p-value is 1 and
