...
def _clean_inputs(lp):
    ...
    lp : A `scipy.optimize._linprog_util._LPProblem` consisting of the following fields:
        ...
        bounds : 2D array
            ...
            rows and upper bounds in the 2nd. Unbounded variables have lower
    ...
    # Convert the nan-s in the 1st rows to -np.inf and in the 2nd rows
    # to np.inf
    ...
    if A.shape[0] > 0:
        ...
        if np.any(np.isinf(x)):  # if an unconstrained variable has no bound
            ...
            message = ("If feasible, the problem is (trivially) unbounded "
                       "due  to a zero rows in the constraint matrices. If "
...
def _get_Abc(lp, c0):
    ...
    lp : A `scipy.optimize._linprog_util._LPProblem` consisting of the following fields:
        ...
        b_eq : 1D array, optional
            ...
            the corresponding element of ``b_eq``.
        bounds : 2D array
            The bounds of ``x``, lower bounds in the 1st rows, upper
            bounds in the 2nd rows. The bounds are possibly tightened
