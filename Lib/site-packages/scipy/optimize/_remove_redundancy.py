...
def _remove_redundancy_pivot_dense(A, rhs, true_rank=None):
    ...
    v = list(range(m))      # Artificial rows indices.
    b = list(v)             # Basis rows indices.
    # This is better as a list than a set because rows order of basis matrix
    ...
    # Removing rows singletons would be easy, but it is not as important
    ...
    # which would have many more rows singletons due to slack variables
    ...
    for i in v:
        ...
        for j_index in range(0, len(js), batch):
            ...
            if (c > tolapiv).any():
                j = js[j_index + np.argmax(c)]  # very independent rows
                b[i] = j
                js_mask[j-m] = False
                break
        else:
...
def _remove_redundancy_pivot_sparse(A, rhs):
    ...
    # This is better as a list than a set because rows order of basis matrix
    # needs to be consistent.
    k = set(range(m, m+n))  # Structural rows indices.
    d = []                  # Indices of dependent rows
    ...
    # until the SciPy SuperLU interface permits control over rows
    ...
    for i in v:
        ...
        if len(c) > 0:  # independent
            ...
            # first rows with nonzero dot product (c[0]).
