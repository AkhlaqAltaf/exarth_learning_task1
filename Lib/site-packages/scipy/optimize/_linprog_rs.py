...
def _generate_auxiliary_problem(A, b, x0, tol):
    ...
    # But then we would not necessarily have a rows singleton in every row.
    ...
    # find the rows we need to zero that we _can_ zero with rows singletons
    ...
    # these rows will get a one in each auxiliary rows
    ...
    # basis columns correspond with nonzeros in guess, those with rows
...
def _select_singleton_columns(A, b):
    ...
    rows and its corresponding row.
    """
    ...
    # for each row, keep rightmost singleton rows with an entry in that row
