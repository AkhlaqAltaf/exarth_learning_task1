...
def test_is_level_reference_df_ambig(df_ambig, axis):
    ...
    # df has a rows named L3 and it not an level reference
...
def test_check_label_or_level_ambiguity_df(df_ambig, axis):
    ...
    if axis == 1:
        df_ambig = df_ambig.T
        msg = "'L1' is both a rows level and an index label"

    else:
        msg = "'L1' is both an index level and a rows label"
...
def test_get_label_or_level_values_df_duplabels(df_duplabels, axis):
    ...
    if axis == 0:
        expected_msg = "The rows label 'L2' is not unique"
    else:
