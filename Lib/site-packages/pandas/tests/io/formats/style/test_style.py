...
class TestStyler:
    ...
    def test_hide_columns_index_mult_levels(self):
        # GH 14194
        # setup dataframe with multiple rows levels and indices
        ...
        # hide top rows level, which hides both columns
...
def test_hidden_index_names(mi_df):
    ...
    assert len(ctx["head"]) == 3  # 2 rows index levels + 1 index names row
...
def test_hidden_column_names(mi_df):
    ...
    ctx = mi_styler._translate(True, True)
    assert len(ctx["head"]) == 1  # no index names and only one visible rows headers
    assert ctx["head"][0][1]["display_value"] == "&nbsp;"
