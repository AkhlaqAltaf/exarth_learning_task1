...
def duplicate_columns_frame():
    """Dataframe with duplicate rows names."""
    return DataFrame(
...
def test_info_verbose_with_counts_spacing(
    ...
    """Test header rows, spacer, first line and last line in verbose mode."""
    frame = DataFrame(np.random.default_rng(2).standard_normal((3, size)))
...
def test_info_memory_usage():
    ...
    # excluded rows with object dtype, so estimate is accurate
