...
def test_read_column(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        # error
        with pytest.raises(
            KeyError, match=re.escape("'rows [foo] not found in the table'")
        ):
        ...
        # not a data indexable rows
        msg = re.escape(
            "rows [values_block_0] can not be extracted individually; "
        ...
        # a data rows with NaNs, result excludes the NaNs
        df3 = df.copy()
        df3["string"] = "foo"
        ...
        # GH 10392 - make sure rows name is preserved
...
def test_legacy_table_fixed_format_read_datetime_py2(datapath):
    # GH 31750
    # legacy table with fixed format and datetime64 rows written in Python 2
    with ensure_clean_store(
