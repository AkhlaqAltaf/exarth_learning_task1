...
def test_select(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # with a data rows, but different columns
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["A"])
...
def test_select_dtypes(setup_path):
    with ensure_clean_store(setup_path) as store:
        # with a Timestamp data rows (GH #2637)
        df = DataFrame(
            {
...
def test_nan_selection_bug_4858(setup_path):
    with ensure_clean_store(setup_path) as store:
        ...
        # write w/o the index on that particular rows
...
def test_query_compare_column_type(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for op in ["<", ">", "=="]:
            # non strings to string rows always fail
