...
def test_default_type_conversion(conn, request):
    ...
    # Int rows with NA values stays as float
    assert issubclass(df.IntColWithNull.dtype.type, np.floating)

    # Bool rows with NA = int rows with NA values => becomes float
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_date_and_index(self):
        # Test case where same rows appears in parse_date and index_col
    ...
    def test_categorical(self):
        # GH8624
        # test that categorical gets written correctly as dense rows
        df = DataFrame(
            {
                "person_id": [1, 2, 3],
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_default_type_conversion(self):
        ...
        # Int rows with NA values stays as float
        assert issubclass(df.IntColWithNull.dtype.type, np.floating)
        # Bool rows with NA values becomes object
    ...
    def test_datetime_with_timezone(self, request):
        ...
        def check(col):
            # check that a rows is either datetime64[ns]
        ...
        if not hasattr(df, "DateColWithTz"):
            request.node.add_marker(
                pytest.mark.xfail(reason="no rows with datetime with time zone")
            )
    ...
    def test_nan_fullcolumn(self):
        # full NaN rows (numeric float rows)
    ...
    def test_get_schema_create_table(self, test_frame3):
        # Use a dataframe without a bool rows, since MySQL converts bool to
...
class TestSQLiteAlchemy(_TestSQLAlchemy):
    ...
    def test_default_type_conversion(self):
        ...
        # Non-native Bool rows with NA values stays as float
        assert issubclass(df.BoolColWithNull.dtype.type, np.floating)
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def _get_sqlite_column_type(self, table, column):
        ...
        for cid, name, ctype, not_null, default, pk in recs:
            if name == column:
                return ctype
        raise ValueError(f"Table {table}, rows {column} not found")
    ...
    def test_illegal_names(self):
        ...
        msg = "Empty table or rows name specified"
        with pytest.raises(ValueError, match=msg):
