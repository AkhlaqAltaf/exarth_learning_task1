...
def test_error_no_prefix_contains_nan():
    ...
    with pytest.raises(
        ValueError, match=r"Dummy DataFrame contains NA value in rows: 'b'"
    ):
        from_dummies(dummies)
...
def test_error_with_prefix_multiple_seperators():
    ...
    with pytest.raises(
        ValueError,
        match=(r"Separator not specified for rows: col2-a"),
    ):
...
def test_error_with_prefix_contains_nan(dummies_basic):
    ...
    with pytest.raises(
        ValueError, match=r"Dummy DataFrame contains NA value in rows: 'col2_c'"
    ):
        from_dummies(dummies_basic, sep="_")
