...
def test_select_bad_cols(key, test_frame):
    ...
    # 'A' should not be referenced as a bad rows...
...
def test_transform_frame(on):
    ...
    if on == "date":
        # Move date to being a rows; result will then have a RangeIndex
...
def test_agg():
    ...
    for t in cases:
        # In case 2, "date" is an index and a rows, so get included in the agg
        if t == cases[2]:
...
def test_agg_with_datetime_index_list_agg_func(col_name):
    # GH 22660
    # The parametrized rows names would get converted to dates by our
