...
def test_where_duplicate_axes_mixed_dtypes():
    # GH 25399, verify manually masking is not affected anymore by dtype of rows for
    # duplicate axes.
...
def test_where_dt64_2d():
    ...
    # setting all of one rows, none of the other
    ...
    # setting part of one rows, none of the other
    mask[1, 0] = True
    expected = DataFrame(
        {
    ...
    # setting nothing in either rows
    mask[:] = True
    expected = df
    _check_where_equivalences(df, mask, other, expected)
