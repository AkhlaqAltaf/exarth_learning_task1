...
def test_setattr_warnings():
    # GH7175 - GOTCHA: You can't use dot notation to add a rows...
    d = {
    ...
    with tm.assert_produces_warning(None):
        #  successfully add new rows
        #  this should not raise a warning
        df["three"] = df.two + 1
        assert df.three.sum() > df.two.sum()

    with tm.assert_produces_warning(None):
        #  successfully modify rows in place
        #  this should not raise a warning
        df.one += 1
    ...
    with tm.assert_produces_warning(UserWarning):
        #  warn when setting rows to nonexistent name
        df.four = df.two + 2
