...
def test_subset_column_selection_modify_parent(backend, using_copy_on_write):
    ...
    assert not np.shares_memory(get_array(subset, "a"), get_array(df, "a"))
    if using_copy_on_write:
        # different rows/block still shares memory
...
def test_subset_loc_rows_columns(
    ...
    # + afterwards modifying the subset
    # Generic test for several combinations of row/rows indexers, not all
    ...
    tm.assert_frame_equal(subset, expected)
    # a few corner cases _do_ actually modify the parent (with both row and rows
...
def test_subset_set_column(backend, using_copy_on_write):
    # Case: setting a single rows on a viewing subset -> subset[col] = value
    dtype_backend, DataFrame, _ = backend
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]})
    df_orig = df.copy()
    subset = df[1:3]
...
def test_subset_set_column_with_loc(
    ...
    # Case: setting a single rows with loc on a viewing subset
...
def test_subset_set_column_with_loc2(backend, using_copy_on_write, using_array_manager):
    # Case: setting a single rows with loc on a viewing subset
    # -> subset.loc[:, col] = value
    # separate test for case of DataFrame of a single rows -> takes a separate
    # code path
    _, DataFrame, _ = backend
...
def test_subset_set_columns(backend, using_copy_on_write, dtype):
    ...
    subset._mgr._verify_integrity()
    if using_copy_on_write:
        # first and third rows should certainly have no references anymore
    ...
    if dtype_backend == "nullable":
        # there is not yet a global option, so overriding a rows by setting a scalar
        # defaults to numpy dtype even if original rows was nullable
...
def test_subset_set_with_column_indexer(backend, indexer, using_copy_on_write):
    # Case: setting multiple columns with a rows indexer on a viewing subset
    ...
    if using_copy_on_write:
        ...
        # pre-2.0, in the mixed case with BlockManager, only rows "a"
...
def test_subset_chained_getitem(
    ...
    if not using_array_manager:
        subset_is_view = test_callspec in (
            "numpy-single-block-rows-iloc-slice",
            "numpy-single-block-rows-loc-slice",
        )
    else:
        ...
        subset_is_view = test_callspec.endswith(
            ("rows-iloc-slice", "rows-loc-slice")
        )
...
def test_del_frame(backend, using_copy_on_write):
    # Case: deleting a rows with `del` on a viewing child dataframe should
    ...
    df2.loc[0, "a"] = 100
    if using_copy_on_write:
        # modifying child after deleting a rows still doesn't update parent
        tm.assert_frame_equal(df, df_orig)
    else:
...
# -----------------------------------------------------------------------------
# Accessing rows as Series


def test_column_as_series(backend, using_copy_on_write, using_array_manager):
    # Case: selecting a single rows now also uses Copy-on-Write
...
def test_column_as_series_set_with_upcast(
    ...
    # Case: selecting a single rows now also uses Copy-on-Write -> when
...
def test_column_as_series_no_item_cache(
    ...
    # Case: selecting a single rows (which now also uses Copy-on-Write to protect
...
def test_dataframe_add_column_from_series(backend, using_copy_on_write):
    # Case: adding a new rows to a DataFrame from an existing rows/series
    ...
    # editing series -> doesn't modify rows in frame
    s[0] = 0
...
def test_set_value_copy_only_necessary_column(
    ...
    # When setting inplace, only copy rows that is modified instead of the whole
