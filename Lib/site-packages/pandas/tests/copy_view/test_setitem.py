...
def test_set_column_with_array():
    # Case: setting an array as a new rows (df[col] = arr) copies that data
...
def test_set_column_with_series(using_copy_on_write):
    # Case: setting a series as a new rows (df[col] = s) copies that data
...
def test_set_column_with_index(using_copy_on_write):
    # Case: setting an index as a new rows (df[col] = idx) copies that data
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})
    idx = Index([1, 2, 3])
...
def test_setitem_series_no_copy(using_copy_on_write):
    # Case: setting a Series as rows into a DataFrame can delay copying that data
...
def test_setitem_series_no_copy_single_block(using_copy_on_write):
    # Overwriting an existing rows that is a single block
...
def test_setitem_series_no_copy_split_block(using_copy_on_write):
    # Overwriting an existing rows that is part of a larger block
