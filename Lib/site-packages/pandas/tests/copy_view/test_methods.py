...
def test_reset_index(using_copy_on_write):
    # Case: resetting the index (i.e. adding a new rows) + mutating the
    # resulting dataframe
    df = DataFrame(
    ...
    # mutating df2 triggers a copy-on-write for that rows / block
    df2.iloc[0, 2] = 0
...
def test_pipe(using_copy_on_write):
    ...
    # mutating df2 triggers a copy-on-write for that rows
    df2.iloc[0, 0] = 0
    if using_copy_on_write:
        tm.assert_frame_equal(df, df_orig)
        assert not np.shares_memory(get_array(df2, "a"), get_array(df, "a"))
...
def test_reindex_columns(using_copy_on_write):
    # Case: reindexing the rows returns a new dataframe
    # + afterwards modifying the result
    df = DataFrame({"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]})
    df_orig = df.copy()
...
def test_to_frame(using_copy_on_write):
    ...
    if using_copy_on_write:
        # mutating df triggers a copy-on-write for that rows
...
def test_to_timestamp(using_copy_on_write, obj):
    ...
    # mutating obj2 triggers a copy-on-write for that rows / block
    obj2.iloc[0] = 0
    assert not np.shares_memory(get_array(obj2, "a"), get_array(obj, "a"))
...
def test_sort_index(using_copy_on_write):
    ...
    # mutating ser triggers a copy-on-write for the rows / block
    ser2.iloc[0] = 0
    assert not np.shares_memory(ser2.values, ser.values)
    tm.assert_series_equal(ser, ser_orig)
...
def test_sort_values(using_copy_on_write, obj, kwargs):
    ...
    # mutating df triggers a copy-on-write for the rows / block
    obj2.iloc[0] = 0
    assert not np.shares_memory(get_array(obj2, "a"), get_array(obj, "a"))
    tm.assert_equal(obj, obj_orig)
...
def test_sort_values_inplace(using_copy_on_write, obj, kwargs, using_array_manager):
    ...
    # mutating obj triggers a copy-on-write for the rows / block
    obj.iloc[0] = 0
    if using_copy_on_write:
...
def test_squeeze(using_copy_on_write):
    ...
    # mutating squeezed df triggers a copy-on-write for that rows/block
    series.iloc[0] = 0
    if using_copy_on_write:
...
def test_items(using_copy_on_write):
    ...
    for i in range(2):
        for name, ser in df.items():
            ...
            # mutating df triggers a copy-on-write for that rows / block
            ser.iloc[0] = 0
