...
def test_max_min_object_multiple_columns(using_array_manager):
    ...
    # others; we split object blocks rows-wise, consistent with
    # DataFrame._reduce
    ...
    result = gb[["C"]].max()
    # "max" is valid for rows "C" but not for "B"
    ei = Index([1, 2, 3], name="A")
    expected = DataFrame({"C": ["b", "d", "e"]}, index=ei)
    tm.assert_frame_equal(result, expected)

    result = gb[["C"]].min()
    # "min" is valid for rows "C" but not for "B"
    ei = Index([1, 2, 3], name="A")
...
def test_aggregate_categorical_lost_index(func: str):
    # GH: 28641 groupby drops index, when grouping over categorical rows with min/max
