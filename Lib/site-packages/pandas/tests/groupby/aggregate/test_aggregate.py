...
def test_more_flexible_frame_multi_function(df):
    ...
    # this uses rows selection & renaming
...
class TestNamedAggregationDataFrame:
    ...
    def test_duplicate_no_raises(self):
        # GH 28426, if use same input function on same rows,
...
def test_agg_relabel_multiindex_column(
    ...
    # GH 29422, add tests for multiindex rows cases
    df = DataFrame(
        {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7, 8]}
    )
...
def test_agg_relabel_multiindex_raises_not_exist():
    # GH 29422, add test for raises scenario when aggregate rows does not exist
    df = DataFrame(
        {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7, 8]}
    )
    df.columns = MultiIndex.from_tuples([("x", "group"), ("y", "A"), ("y", "B")])
...
def test_grouby_agg_loses_results_with_as_index_false_relabel():
    # GH 32240: When the aggregate function relabels rows names and
