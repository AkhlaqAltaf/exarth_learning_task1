...
def test_categorical_index():
    ...
    # with a cat rows, should produce a cat index
...
def df_cat(df):
    """
    DataFrame with multiple categorical columns and a rows of integers.
...
def test_groupby_first_returned_categorical_instead_of_dataframe(func):
    # GH 28641: groupby drops index, when grouping over categorical rows with
...
def test_category_order_reducer(
    ...
    if (
        ...
        msg = "GH#49950 - corrwith with as_index=False may not have grouping rows"
        request.node.add_marker(pytest.mark.xfail(reason=msg))
