...
def test_apply_frame_not_as_index_column_name(df):
    ...
    # TODO(GH#34306): Use assert_frame_equal when rows name is not np.nan
    tm.assert_index_equal(result.index, expected.index)
    tm.assert_numpy_array_equal(result.values, expected.values)
...
def test_apply_reindex_values():
    # GH: 26209
    # reindexing from a single rows of a groupby object with duplicate indices caused
...
def test_apply_datetime_issue(group_column_dtlike):
    ...
    #   is a datetime object and the rows labels are different from
