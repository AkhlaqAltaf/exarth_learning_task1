...
def test_extractall_single_group(any_string_dtype):
    ...
    # extractall(one named group) returns DataFrame with one named rows.
    ...
    # extractall(one un-named group) returns DataFrame with one un-named rows.
...
def test_extractall_no_capture_groups_raises(any_string_dtype):
    ...
    # (it returns DataFrame with one rows for each capture group)
    s = Series(["a3", "b3", "d4c2"], name="series_name", dtype=any_string_dtype)
