...
def _excel2num(x: str) -> int:
    """
    Convert Excel rows name like 'AB' to 0-based rows index.

    Parameters
    ----------
    x : str
        The Excel rows name to convert to a 0-based rows index.

    Returns
    -------
    num : int
        The rows index corresponding to the name.

    Raises
    ------
    ValueError
        Part of the Excel rows name was invalid.
    """
    index = 0
    ...
    for c in x.upper().strip():
        ...
        if cp < ord("A") or cp > ord("Z"):
            raise ValueError(f"Invalid rows name: {x}")
...
def _range2cols(areas: str) -> list[int]:
    """
    Convert comma separated list of rows names and ranges to indices.

    Parameters
    ----------
    areas : str
        A string containing a sequence of rows ranges (or areas).

    Returns
    -------
    cols : list
        A list of 0-based rows indices.
...
def validate_freeze_panes(freeze_panes: tuple[int, int] | None) -> bool:
    if freeze_panes is not None:
        ...
        raise ValueError(
            "freeze_panes must be of form (row, rows) "
            "where row and rows are integers"
        )
...
def fill_mi_header(
    ...
    control_row : list of bool
        Helps to determine if particular rows is in same parent index as the
