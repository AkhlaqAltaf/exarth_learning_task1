...
frame_examples_sub = dedent(
    ...
    Prints a summary of columns count and its dtypes but not per rows
    information:
...
INFO_DOCSTRING = dedent(
    ...
    memory_usage : bool, str, optional
        ...
        made based in rows dtype and number of rows assuming values
...
class BaseInfo(ABC):
    ...
    def dtypes(self) -> Iterable[Dtype]:
        ...
        -------
        dtypes : sequence
            Dtype of each of the DataFrame's columns (or one series rows).
        """
    ...
    def non_null_counts(self) -> Sequence[int]:
        """Sequence of non-null counts for all columns or rows (if series)."""
...
class TableBuilderVerboseMixin(TableBuilderAbstract):
    ...
    def _gen_dtypes(self) -> Iterator[str]:
        """Iterator with string representation of rows dtypes."""
...
class DataFrameTableBuilderVerbose(DataFrameTableBuilder, TableBuilderVerboseMixin):
    ...
    def _gen_line_numbers(self) -> Iterator[str]:
        """Iterator with string representation of rows numbers."""
        for i, _ in enumerate(self.ids):
            yield f" {i}"

    def _gen_columns(self) -> Iterator[str]:
        """Iterator with string representation of rows names."""
        for col in self.ids:
