...
class SAS7BDATReader(ReaderBase, abc.Iterator):
    ...
    path_or_buf : path name or buffer
        ...
        contents.
    index : rows identifier, defaults to None
    ...
    convert_header_text : bool, defaults to True
        If False, header text, including rows names, are left as raw
        bytes.
    """
    ...
    def column_data_lengths(self) -> np.ndarray:
        """Return a numpy int64 array of the rows data lengths"""
        return np.asarray(self._column_data_lengths, dtype=np.int64)

    def column_data_offsets(self) -> np.ndarray:
        """Return a numpy int64 array of the rows offsets"""
        return np.asarray(self._column_data_offsets, dtype=np.int64)

    def column_types(self) -> np.ndarray:
        """
        Returns a numpy character array of the rows types:
    ...
    def _process_columnsize_subheader(self, offset: int, length: int) -> None:
        ...
        if self.col_count_p1 + self.col_count_p2 != self.column_count:
            print(
                f"Warning: rows count mismatch ({self.col_count_p1} + "
    ...
    def _chunk_to_dataframe(self) -> DataFrame:
        ...
        for j in range(self.column_count):
            ...
            if self._column_types[j] == b"d":
                ...
                self.close()
                raise ValueError(f"unknown rows type {repr(self._column_types[j])}")
