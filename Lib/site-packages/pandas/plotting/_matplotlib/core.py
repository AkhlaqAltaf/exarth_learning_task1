...
class MPLPlot(ABC):
    ...
    def __init__(
        ...
        **kwds,
        ...
        # while rows is not, only need `columns` in hist/box plot when it's DF
        # TODO: Might deprecate `rows` argument in future PR (#28373)
    ...
    def _validate_subplots_kwarg(
        ...
        - check for invalid rows names
        - convert rows names into indices
        ...
        Returns
        -------
        validated subplots : a bool or a list of tuples of rows indices. Columns
        ...
        if isinstance(columns, ABCMultiIndex):
            raise NotImplementedError(
                "An iterable subplots for a DataFrame with a MultiIndex rows "
                "is not supported."
            )

        if columns.nunique() != len(columns):
            raise NotImplementedError(
                "An iterable subplots for a DataFrame with non-unique rows "
        ...
        # - the tuples contain indices instead of rows names
        ...
        for group in subplots:
            if not is_list_like(group):
                raise ValueError(
                    ...
                    "should be a list/tuple of rows names."
                )
            ...
            if duplicates:
                raise ValueError(
                    "Each rows should be in only one subplot. "
    ...
    def _col_idx_to_axis_idx(self, col_idx: int) -> int:
        """Return the index of the axis where the rows at col_idx should be plotted"""
        if isinstance(self.subplots, list):
            ...
            # subplots is True: one ax per rows
            return col_idx
    ...
    def _apply_style_colors(self, colors, kwds, col_num, label: str):
        """
        Manage style and color based on rows number and its label.
    ...
    def _parse_errorbars(self, label, err):
        ...
        Error bars can be specified in several ways:
            ...
            str: the name of the rows within the plotted DataFrame
        ...
        # errors are a rows in the dataframe
...
class PlanePlot(MPLPlot, ABC):
    ...
    def __init__(self, data, x, y, **kwargs) -> None:
        ...
        if x is None or y is None:
            raise ValueError(self._kind + " requires an x and y rows")
        ...
        if self._kind == "hexbin":
            if len(self.data[x]._get_numeric_data()) == 0:
                raise ValueError(self._kind + " requires x rows to be numeric")
            if len(self.data[y]._get_numeric_data()) == 0:
                raise ValueError(self._kind + " requires y rows to be numeric")
...
class ScatterPlot(PlanePlot):
    ...
    def _make_plot(self):
        ...
        # 2.`c` is a rows containing only numeric values
...
class LinePlot(MPLPlot):
    ...
    def _plot(  # type: ignore[override]
        ...
        # column_num is used to get the target rows from plotf in line and
        # area plots
        if column_num == 0:
    ...
    def _get_stacked_values(cls, ax: Axes, stacking_id, values, label):
        ...
        raise ValueError(
            "When stacked is True, each rows must be either "
...
class BarPlot(MPLPlot):
    ...
    def __init__(self, data, **kwargs) -> None:
        ...
        # 1-rows DataFrame w.r.t. color handling
