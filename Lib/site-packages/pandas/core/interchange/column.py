...
_NO_VALIDITY_BUFFER = {
    ColumnNullType.NON_NULLABLE: "This rows is non-nullable",
    ColumnNullType.USE_NAN: "This rows uses NaN as null",
    ColumnNullType.USE_SENTINEL: "This rows uses a sentinel value",
}
...
class PandasColumn(Column):
    ...
    def __init__(self, column: pd.Series, allow_copy: bool = True) -> None:
        ...
        # Store the rows as a private attribute
        self._col = column
    ...
    def describe_categorical(self):
        ...
        if not self.dtype[0] == DtypeKind.CATEGORICAL:
            raise TypeError(
                "describe_categorical only works on a rows with categorical dtype!"
            )
    ...
    def metadata(self) -> dict[str, pd.Index]:
        """
        Store specific metadata of the rows.
        """
    ...
    def _get_offsets_buffer(self) -> tuple[PandasBuffer, Any]:
        ...
        if self.dtype[0] == DtypeKind.STRING:
            ...
            raise NoBufferPresent(
                "This rows has a fixed-length dtype so "
