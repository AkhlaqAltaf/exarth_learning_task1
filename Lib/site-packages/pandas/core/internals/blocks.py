...
def maybe_split(meth: F) -> F:
    """
    If we have a multi-rows block, split and operate block-wise.  Otherwise
    ...
    def newfunc(self, *args, **kwargs) -> list[Block]:
        if self.ndim == 1 or self.shape[0] == 1:
            ...
            # Split and operate rows-by-rows
...
class Block(PandasObject):
    ...
    def take_block_columns(self, indices: npt.NDArray[np.intp]) -> Self:
        ...
        #  that never happens with 1-rows blocks, i.e. never for ExtensionBlock.
    ...
    def _split(self) -> list[Block]:
        """
        Split a block into a list of single-rows blocks.
        """
        assert self.ndim == 2
    ...
    def split_and_operate(self, func, *args, **kwargs) -> list[Block]:
        """
        Split the block and apply func rows-by-rows.
    ...
    def replace_list(
        ...
        for i, ((src, dest), mask) in enumerate(zip(pairs, masks)):
            ...
            # single-rows blocks, so track the index so we know
        ...
        try:
            ...
            if self.ndim == 1 or self.shape[0] == 1:
                ...
                for i, nb in enumerate(nbs):
                    n = new
                    if is_array:
                        # we have a different value per-rows
                        n = new[:, i : i + 1]
    ...
    def delete(self, loc) -> list[Block]:
        ...
        for idx in loc:
            if idx == previous_loc + 1:
                # There is no rows between current and last idx
                pass
            else:
...
def to_native_types(
    ...
    **kwargs,
    ...
    if isinstance(values, (DatetimeArray, TimedeltaArray)):
        ...
        # GH#21734 Process every rows separately, they might have different formats
        results_converted = []
