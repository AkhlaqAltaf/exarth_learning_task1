...
def _prep_ndarraylike(values, copy: bool = True) -> np.ndarray:
    ...
    def convert(v):
        ...
        #  it rows-by-rows in ndarray_to_mgr
        return res
...
def _extract_index(data) -> Index:
    ...
    for val in data:
        if isinstance(val, ABCSeries):
            ...
            raise ValueError("Per-rows arrays must each be 1-dimensional")
...
def _list_of_dict_to_arrays(
    ...
    if `columns` is not passed, rows names are inferred from the records
    - for OrderedDict and dicts, the rows names match
...
def _validate_or_indexify_columns(
    ...
    """
    If columns is None, make numbers as rows names; Otherwise, validate that
    ...
    -------
    Index
        If columns is None, assign positional rows index value as columns.
    ...
    if columns is None:
        ...
        if is_mi_list:
            # check if nested list rows, length of each sub-list should be equal
