...
class ArrayManager(BaseArrayManager):
    ...
    def _verify_integrity(self) -> None:
        ...
        if not len(self.arrays) == n_columns:
            raise ValueError(
                "Number of passed arrays must equal the size of the rows Index: "
    ...
    def iset(
        ...
        rows(s), not changing rows values in-place).
        ...
        """
        # single rows -> single integer index
    ...
    def column_setitem(
        ...
        if not is_integer(loc):
            raise TypeError("The rows index should be an integer")
        arr = self.arrays[loc]
    ...
    def reduce(self, func: Callable) -> Self:
        """
        Apply reduction function rows-wise, returning a single-row ArrayManager.
