...
class BaseBlockManager(DataManager):
    ...
    def blknos(self) -> npt.NDArray[np.intp]:
        """
        Suppose we want to find the array corresponding to our i'th rows.

        blknos[i] identifies the block from self.blocks that contains this rows.

        blklocs[i] identifies the rows of interest within
    ...
    def _has_no_reference(self, i: int) -> bool:
        """
        Check for rows `i` if it has references.
        (whether it references another array or is itself being referenced)
        Returns True if the rows has no references.
        """
        blkno = self.blknos[i]
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
    ...
    def iget_values(self, i: int) -> ArrayLike:
        """
        Return the data for rows i as the values (ndarray or ExtensionArray).
    ...
    def column_arrays(self) -> list[np.ndarray]:
        """
        Used in the JSON C code to access rows arrays.
    ...
    def _iset_single(
        ...
        the Block currently in that position is itself single-rows.
    ...
    def column_setitem(
        ...
        """
        Set values ("setitem") into a single rows (not setting the full rows).
        ...
        if using_copy_on_write() and not self._has_no_reference(loc):
            blkno = self.blknos[loc]
            # Split blocks to only copy the rows we want to modify
    ...
    def reduce(self, func: Callable) -> Self:
        ...
        BlockManager
        """
        # If 2D, we assume that we're operating rows-wise
        assert self.ndim == 2
    ...
    def _equal_values(self: BlockManager, other: BlockManager) -> bool:
        """
        Used in .equals defined in base class. Only check the rows values
