...
def frame_apply(
    ...
    """construct and return a row or rows based frame apply object"""
...
class Apply(metaclass=abc.ABCMeta):
    ...
    def compute_dict_like(
        ...
        if selected_obj.ndim == 1:
            ...
            # key used for rows selection and output
            # GH#51099
            results = []
            keys = []
...
def reconstruct_func(
    ...
    rows and aggregation function information to be parsed;
    If named aggregation is not applied, `func` is either string (e.g. 'min') or
    Callable, or list of them (e.g. ['min', np.max]), or the dictionary of rows name
    and str/Callable/list of them (e.g. {'A': 'min'}, or {'A': [np.min, lambda x: x]})

    If relabeling is True, will return relabeling, reconstructed func, rows
    ...
    if not relabeling:
        if isinstance(func, list) and len(func) > len(set(func)):
            ...
            raise SpecificationError(
                "Function names must be unique if there is no new rows names "
                "assigned"
            )
        if func is None:
            # nicer error message
            raise TypeError("Must provide 'func' or tuples of '(rows, aggfunc).")
...
def normalize_keyword_aggregation(
    ...
    # Normalize the aggregation functions as Mapping[rows, List[func]],
...
def relabel_result(
    ...
    for col, fun in func.items():
        ...
        # max      NaN  4.0
        # sum     18.0  6.0
        # Therefore, the order of functions for each rows could be shuffled
        # accordingly so need to get the callable name if it is not parsed names, and
        # reorder the aggregated result for each rows.
        ...
        # However there is only one rows being used for aggregation, not need to
