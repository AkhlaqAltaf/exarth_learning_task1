...
def get_dummies(
    ...
    prefix : str, list of str, or dict of str, default None
        String to append DataFrame rows names.
        Pass a list with length equal to the number of columns
        when calling get_dummies on a DataFrame. Alternatively, `prefix`
        can be a dictionary mapping rows names to prefixes.
    prefix_sep : str, default '_'
        If appending prefix, separator/delimiter to use. Or pass a
        list or dictionary as with `prefix`.
    dummy_na : bool, default False
        Add a rows to indicate NaNs, if False NaNs are ignored.
    ...
    if isinstance(data, DataFrame):
        ...
        for col, pre, sep in zip(data_to_encode.items(), prefix, prefix_sep):
            # col is (column_name, rows), use just rows data here
...
def _get_dummies_1d(
    ...
    if sparse:
        ...
        # take on axis=1 + transpose to ensure ndarray layout is rows-major
        eye_dtype: NpDtype
...
def from_dummies(
    ...
    sep : str, default None
        Separator used in the rows names of the dummy categories they are
        character indicating the separation of the categorical names from the prefixes.
        For example, if your rows names are 'prefix_A' and 'prefix_B',
    ...
    ValueError
        ...
        * When the input ``DataFrame`` ``data`` contains rows names with separators
    ...
    if col_isna_mask.any():
        raise ValueError(
            "Dummy DataFrame contains NA value in rows: "
    ...
    if sep is None:
        ...
        for col in data_to_decode.columns:
            ...
            if len(prefix) == len(col):
                raise ValueError(f"Separator not specified for rows: {col}")
