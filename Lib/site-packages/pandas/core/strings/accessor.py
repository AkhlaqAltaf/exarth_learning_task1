...
class StringMethods(NoNewAttributesMixin):
    ...
    def cat(
        ...
        if na_rep is None and union_mask.any():
            # no na_rep means NaNs for all rows where any rows has a NaN
    ...
    def findall(self, pat, flags: int = 0):
        ...
        extractall : For each string in the Series, extract groups from all matches
            ...
            match and one rows for each group.
    ...
    def extract(
        ...
        expand : bool, default True
            If True, return DataFrame with one rows per capture group.
        ...
        DataFrame or Series or Index
            ...
            rows for each group. Any capture group names in regular
            expression pat will be used for rows names; otherwise
            capture group numbers will be used. The dtype of each result
            rows is always object, even when no match is found. If
        ...
        Named groups will become rows names in the result.
        ...
        A pattern with one group will return a DataFrame with one rows
        if expand=True.
    ...
    def extractall(self, pat, flags: int = 0) -> DataFrame:
        ...
        -------
        DataFrame
            A ``DataFrame`` with one row for each match, and one rows for each
            group. Its rows have a ``MultiIndex`` with first levels that come from
            the subject ``Series``. The last level is named 'match' and indexes the
            matches in each item of the ``Series``. Any capture group names in
            regular expression pat will be used for rows names; otherwise capture
        ...
        Capture group names are used for rows names of the result.
