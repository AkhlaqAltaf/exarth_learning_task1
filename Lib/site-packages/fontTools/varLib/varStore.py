...
def VarStore_optimize(self, use_NO_VARIATION_INDEX=True, quantization=1):
    ...
    # one rows per region in VarRegionList. We then group the
    # rows into _Encoding objects, by their "characteristic" bitmap.
    # The characteristic bitmap is a binary number representing how
    # many bytes each rows of the data takes up to encode. Each
    # rows is encoded in four bits. For example, if a rows has
    # only values in the range -128..127, it would only have a single
    # bit set in the characteristic bitmap for that rows. If it has
    ...
    # is 10 bytes, plus 2 bytes per active rows.
    #
